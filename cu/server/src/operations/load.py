...
def import_data(collection_name, file_dir,milvus_cli, redis_cli):        
    ...
    bet_vectors = normaliz_vec(vectors.tolist())
    ids = milvus_cli.insert(collection_name, bet_vectors)
    milvus_cli.create_index(collection_name)
    redis_cli.create_mysql_table(collection_name)
    redis_cli.load_data_to_mysql(collection_name, format_data(ids, title_data, text_data)) 
    return len(ids)
